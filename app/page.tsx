...
export default function Home() {
  ...
  if (screen === "result") return <><LessonResult score={assessmentScore} onOpenChest={() => setChestOpen(true)} onMap={() => setScreen("roadmap")} />{chestOpen && <div className="chest-modal" role="dialog" aria-modal="true" aria-label="Rương linh vật"><section><span className="chest-rays"/><small>RƯƠNG LEVEL 1 ĐÃ MỞ</small><MascotPortrait id={pendingCompanion}/><h2>Xin chào, {companions[pendingCompanion].name}!</h2><p>Em đã mở khóa <b>{companions[pendingCompanion].kind}</b>. Em có thể chọn bạn ấy hoặc Sói Con cùng đồng hành.</p><button onClick={claimCompanion}><Gift size={18}/> Nhận linh vật</button></section></div>}</>;
